...
- [Atomic Test #9: Masquerading - macOS system utility renamed to a trusted-sounding name](#atomic-test-9-masquerading---macos-system-utility-renamed-to-a-trusted-sounding-name)
...
### Atomic Test #9: Masquerading - macOS system utility renamed to a trusted-sounding name

Copies a legitimate macOS system utility to a non-standard directory under a name resembling a
trusted Apple system process, then executes it from there. Because the copy is the genuine,
signed system binary, this can also be used to slip past application allow-listing rules that
key off binary name/path rather than code signature.

**Supported Platforms:** macOS

**auto_generated_guid:** `8479faf5-842d-4281-a5ee-38b1675be6d7`

#### Inputs

| Name | Description | Type | Default Value |
|------|-------------|------|---------------|
| source_utility | Legitimate macOS system utility to copy and rename. | path | /usr/bin/curl|
| masquerade_path | Path/name to masquerade the utility as. | path | /tmp/com.apple.softwareupdated|

#### Attack Commands: Run with `bash`!

```bash
cp #{source_utility} #{masquerade_path}
#{masquerade_path} --version
```
