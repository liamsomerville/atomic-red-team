...
- [Atomic Test #3: Masquerade as a Legitimate macOS System Process](#atomic-test-3-masquerade-as-a-legitimate-macos-system-process)
...
### Atomic Test #3: Masquerade as a Legitimate macOS System Process

Copies a benign binary to a location outside the standard system directories and renames it to
match a legitimate-sounding macOS system process name (e.g. a background daemon), then executes
it from there. Malware families commonly run under names resembling legitimate macOS processes
(such as `mdworker`, `coreaudiod`, or `com.apple.*`) from unusual paths like `/tmp` or a user's
Library folder in order to blend in with normal process listings.

**Supported Platforms:** macOS

**auto_generated_guid:** `e5e5f2a4-e9b5-4cc3-ab3c-1e8dd69516a1`

#### Inputs

| Name | Description | Type | Default Value |
|------|-------------|------|---------------|
| source_binary | Benign binary to copy and rename. | path | /bin/sleep|
| masquerade_path | Path/name to masquerade the binary as. | path | /tmp/com.apple.coreservices.appleidauthenticationd|

#### Attack Commands: Run with `bash`!

```bash
cp #{source_binary} #{masquerade_path}
#{masquerade_path} 30 &
sleep 1
ps aux | grep "#{masquerade_path}" | grep -v grep
```

#### Cleanup Commands

```bash
pkill -f "#{masquerade_path}" 2>/dev/null
rm -f #{masquerade_path}
```
