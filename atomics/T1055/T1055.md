...
- [Atomic Test #14: Process Injection via lldb (Same-User Process)](#atomic-test-14-process-injection-via-lldb-same-user-process)
...
### Atomic Test #14: Process Injection via lldb (Same-User Process)

Spawns a benign helper process, then attaches to it with the native `lldb` debugger and uses
expression evaluation to invoke `system()` inside the target's address space, causing the target
process to execute an arbitrary command under its own PID. Same-user, non-hardened processes can
be attached to and manipulated this way without additional entitlements, reflecting macOS process
injection techniques that abuse debugging primitives (task_for_pid/ptrace) rather than dylib
injection.

NOTE: on modern macOS the executing terminal/app must have been granted the "Developer Tools"
permission (Privacy & Security) for lldb to attach to another process.

**Supported Platforms:** macOS

**auto_generated_guid:** `269978ab-8013-4fbb-a979-e0b47fe243b1`

#### Inputs

| Name | Description | Type | Default Value |
|------|-------------|------|---------------|
| marker_file | File the injected command creates inside the target process to prove injection succeeded. | path | /tmp/T1055_injected.txt|
...
sleep 300 &
TARGET_PID=$!
lldb -p $TARGET_PID -o 'expr (int)system("touch #{marker_file}")' -o 'detach' -o 'quit'
ls -la #{marker_file}
kill $TARGET_PID
```
